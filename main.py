...
class model_getter:
    ...
    def get_dataloader_args(self):
        if self.device.type == "cuda":
            return dict(batch_size_train=64, batch_size_test=1000, shuffle_train=True, shuffle_test=False, num_workers=2, pin_memory=True) 
        else:
            return dict(batch_size_train=64, batch_size_test=1000, shuffle_train=True, shuffle_test=False)
...
def main():
    ...
    train_test_instance = train_test.train_test_model(model.model_obj, model.device, data_setup_instance.train_loader, data_setup_instance.test_loader)
    train_test_instance.run_epoch(epochs=3)
